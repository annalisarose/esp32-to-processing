...
const char * ssid = "Oak Street Bldg";
const char * password = "232OAK!!";
...
void setup() {
  ...
  pinMode(PulseSensorPurplePin,INPUT);         // pin that will blink to your heartbeat!
...
void loop() {
  ...
  Signal = digitalRead(PulseSensorPurplePin);  // Read the PulseSensor's value.
                                              // Assign this value to the "Signal" variable.

   Serial.println(Signal);// Send the Signal value to Serial Plotter.
    //  Serial.println("Yeah");

   if(Signal == HIGH){                          // If the signal is above "550", then "turn-on" Arduino's on-Board LED.
